...
struct AboutPokemon: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                if let pokemon = pokemon, let typePokemon = pokemon.types[0].type.name, let imageURLPokemon = pokemon.imagem.style.home.front {
                    ...
                    VStack {
                        ...
                        Button(action: {
                            isPokemonDescriptionPresented = true
                        }) {
                            VStack {
                                HStack{
                                    Text("Show more..")
            ...
            .onDisappear{
                self.pokemon = PokemonDetail(id: 0, name: "", weight: 0, types: [Types(type: Tipos(name: .normal))], imagem: Sprites(style: Home(home: Default(front: ""))), height: 0.0, stats: [PokemonStat(baseStat: 0, stat: Stat(name: ""))])
            }
        }
    }
